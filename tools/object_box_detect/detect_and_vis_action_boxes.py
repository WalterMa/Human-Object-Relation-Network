...
def get_dataset(dataset, args):
    if dataset.lower() == 'voca':
        ...
        val_dataset = gdata.HICOClassification(split='test', preload_label=False)
    else:
    ...
    with tqdm(total=len(dataset)) as pbar:
        ...
        for ib, batch in enumerate(val_data):
            ...
            for x, y, im_scale in zip(*batch):
                ...
                # clip to image size
                bboxes = clipper(bboxes[:, 0:3, :], x)
                # rescale to original resolution
                im_scale = im_scale.reshape((-1)).asscalar()
                bboxes *= im_scale
                bboxes = bboxes.reshape((-1, 4)).asnumpy()
                # split ground truths
                img_path = dataset.img_path(idx)
                img = mx.image.imread(img_path, 1)
                img = img.asnumpy().astype('uint8')
                box_scores = scores.reshape((-1)).asnumpy()
                ax = gutils.viz.plot_single_gt_with_realtion(img, bboxes[0], bboxes, box_scores[0:3], thresh=0, ax=ax)
                fig_name = os.path.splitext(os.path.basename(dataset.img_path(idx)))[0]
                plt.title(os.path.basename(dataset.img_path(idx)))
                # plt.show()
                plt.savefig('./tmp/{}.jpg'.format(fig_name))
                plt.cla()
                idx += 1
            pbar.update(len(ctx))
