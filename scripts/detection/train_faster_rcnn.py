...
from libgcv.data.transforms.presets.rcnn import FasterRCNNDefaultTrainTransform
from libgcv.data.transforms.presets.rcnn import FasterRCNNDefaultValTransform
from libgcv.utils.metrics.voc_detection import VOCMApMetric, VOC07MApMetric
from libgcv.utils.metrics.coco_detection import COCODetectionMetric
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='polar',
                        help='Training dataset. Now support polar, voca, voc and coco.')
    ...
    parser.add_argument('--input-gt-box', action='store_true',
                        help='Use gt box in test forward.')
    parser.add_argument('--seed', type=int, default=233,
                        help='Random seed to be fixed.')
    parser.add_argument('--verbose', dest='verbose', action='store_true',
                        help='Print helpful debugging info once set.')
    args = parser.parse_args()
    if args.dataset == 'polar':
...
def validate(net, val_data, ctx, eval_metric):
    ...
    clipper = libgcv.nn.bbox.BBoxClipToImage()
    eval_metric.reset()
    ...
    for batch in val_data:
        ...
        for x, y, im_scale in zip(*batch):
            ...
            ids, scores, bboxes = net(x, gt_box)
            det_ids.append(ids)
...
if __name__ == '__main__':
    ...
    # set test gt box input
    if args.input_gt_box:
        net.set_test_gt_box_input(True)
