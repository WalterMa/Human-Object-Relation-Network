...
class FasterRCNN(RCNN):
    ...
    def __init__(self, features, top_features, classes,
                 ...
                 additional_output=False, test_gt_box_input=False, **kwargs):
        ...
        self._test_gt_box_input = test_gt_box_input
    ...
    def set_test_gt_box_input(self, val=False):
        self._test_gt_box_input = val
    ...
    def hybrid_forward(self, F, x, gt_box=None):
        ...
        gt_box : type, only required during training or test_gt_box_input is true
        ...
        if autograd.is_training():
            ...
            num_roi = self._num_sample
            roi_batchid = F.arange(0, self._max_batch, repeat=num_roi)
        elif self._test_gt_box_input:
            rpn_box = gt_box
            roi_batchid = F.arange(0, self._max_batch).reshape((-1, 1, 1)).\
                broadcast_like(gt_box.slice(begin=(None, None, 0), end=(None, None, 1)))
        else:
            _, rpn_box = self.rpn(feat, F.zeros_like(x))
            num_roi = self._rpn_test_post_nms
            roi_batchid = F.arange(0, self._max_batch, repeat=num_roi)
        ...
        cls_pred = cls_pred.reshape((self._max_batch, -1, self.num_class + 1))
        # box_pred (B * N, C * 4) -> (B, N, C, 4)
        box_pred = box_pred.reshape((self._max_batch, -1, self.num_class, 4))
        ...
        if self._test_gt_box_input:
            return ids, scores, gt_box.tile(reps=(1, self.num_class, 1))
...
def faster_rcnn_resnet50_v1b_voca(pretrained=False, pretrained_base=True, **kwargs):
    r"""Faster RCNN model from the paper
    "Ren, S., He, K., Girshick, R., & Sun, J. (2015). Faster r-cnn: Towards
    real-time object detection with region proposal networks"
    ...
    Examples
    --------
    >>> model = get_faster_rcnn_resnet50_v1b_voca(pretrained=True)
    >>> print(model)
    """
    from ..resnetv1b import resnet50_v1b
    ...
    train_patterns = '|'.join(['.*dense', '.*rpn', '.*down(2|3|4)_conv', '.*layers(2|3|4)_conv'])
    return get_faster_rcnn(
        name='resnet50_v1b', dataset='voca', pretrained=pretrained,
        features=features, top_features=top_features, classes=classes,
        short=600, max_size=1000, train_patterns=train_patterns,
        nms_thresh=0.3, nms_topk=400, post_nms=100,
        roi_mode='align', roi_size=(14, 14), stride=16, clip=None,
