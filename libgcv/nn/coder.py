...
class MultiClassEncoderWithoutNeg(gluon.HybridBlock):
    """Encode classification training target given matching results.
    ...
    """
    def __init__(self, ignore_label=-1):
        super(MultiClassEncoderWithoutNeg, self).__init__()
        self._ignore_label = ignore_label
...
class MultiPerClassEncoder(gluon.HybridBlock):
    """Encode classification training target given matching results.
    ...
    """
    def __init__(self, ignore_label=-1):
        super(MultiPerClassEncoder, self).__init__()
        self._ignore_label = ignore_label
    ...
    def hybrid_forward(self, F, samples, matches, refs):
        ...
        samples: (B, N), value +1 (positive), -1 (negative), 0 (ignore)
        matches: (B, N), value range [0, M)
        refs: (B, M, C), value range [0, num_fg_class), excluding background

        Returns
        -------
        targets: (B, N, C), value range [0, 1], excluding background
        weights: (B, N, 1), value [0, 1], only positive samples

        """
        # samples (B, N) (+1, -1, 0: ignore), matches (B, N) [0, M), refs (B, M)
        # reshape refs (B, M, C) -> (B, 1, M, C) -> (B, N, M, C)
        refs = F.repeat(refs.reshape((0, 1, -1, refs.shape[2])), axis=1, repeats=matches.shape[1])
        # ids (B, N, M, C) -> (B, N, C), value [0, M + 1), 0 reserved for background class
        matches = F.repeat(matches.reshape((0, 0, 1)), axis=2, repeats=refs.shape[-1])
        target_ids = F.pick(refs, matches, axis=2)
        target_ids = F.broadcast_mul(target_ids, (samples > 0.5).reshape((0, 0, 1)))
        # weights = F.logical_or((samples > 0.5), (samples < -0.5)).reshape((0, 0, 1))
        weights = (samples > 0.5).reshape((0, 0, 1))
