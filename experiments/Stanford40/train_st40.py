...
from libgcv.utils.metrics.voc_classification import VOCClsMApMetric
...
def get_dataset(dataset, args):
    if dataset.lower() == 'st40':
        train_dataset = gdata.Stanford40Action(split='train', augment_box=True, load_box=True)
        val_dataset = gdata.Stanford40Action(split='test', load_box=True)
        val_metric = VOCClsMApMetric(class_names=val_dataset.classes)
    else:
